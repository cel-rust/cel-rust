// @generated by `cargo run -p conformance --bin generate`.
// DO NOT EDIT MANUALLY.

// Source file: proto2
// Description: Protocol buffer version 2 tests.  See notes for the available set of protos for tests.

// Section: literal_singular
// Literals with singular fields set.
mod literal_singular {
    use conformance::runner::run_test;
    use dedent::dedent;

    // Test: int64_nocontainer
    #[should_panic]
    #[test]
    fn int64_nocontainer() {
        run_test(&dedent!(
            r#"
                expr: "cel.expr.conformance.proto2.TestAllTypes{single_int64: 17}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] { single_int64: 17 }
                  }
                }
            "#
        ));
    }

    // Test: int32
    #[should_panic]
    #[test]
    fn int32() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_int32: -34}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] { single_int32: -34 }
                  }
                }
            "#
        ));
    }

    // Test: int32_eq_uint
    #[should_panic]
    #[test]
    fn int32_eq_uint() {
        run_test(&dedent!(
            r#"
                container: "google.protobuf"
                expr: "Int32Value{value: 34} == dyn(UInt64Value{value: 34u})"
                value { bool_value: true }
            "#
        ));
    }

    // Test: not_int32_eq_uint
    #[should_panic]
    #[test]
    fn not_int32_eq_uint() {
        run_test(&dedent!(
            r#"
                container: "google.protobuf"
                expr: "Int32Value{value: 34} == dyn(UInt64Value{value: 18446744073709551615u})"
                value { bool_value: false }
            "#
        ));
    }

    // Test: int32_eq_double
    #[should_panic]
    #[test]
    fn int32_eq_double() {
        run_test(&dedent!(
            r#"
                container: "google.protobuf"
                expr: "Int32Value{value: 34} == dyn(DoubleValue{value: 34.0})"
                value { bool_value: true }
            "#
        ));
    }

    // Test: not_int32_eq_double
    #[should_panic]
    #[test]
    fn not_int32_eq_double() {
        run_test(&dedent!(
            r#"
                container: "google.protobuf"
                expr: "Int32Value{value: 34} == dyn(DoubleValue{value: -9223372036854775809.0})"
                value { bool_value: false }
            "#
        ));
    }

    // Test: int64
    #[should_panic]
    #[test]
    fn int64() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_int64: 17}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] { single_int64: 17 }
                  }
                }
            "#
        ));
    }

    // Test: uint32
    #[should_panic]
    #[test]
    fn uint32() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_uint32: 1u}"
                value {
                  object_value: {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] { single_uint32: 1 }
                  }
                }
            "#
        ));
    }

    // Test: uint32_eq_int
    #[should_panic]
    #[test]
    fn uint32_eq_int() {
        run_test(&dedent!(
            r#"
                container: "google.protobuf"
                expr: "UInt32Value{value: 34u} == dyn(Int64Value{value: 34})"
                value { bool_value: true }
            "#
        ));
    }

    // Test: not_uint32_eq_int
    #[should_panic]
    #[test]
    fn not_uint32_eq_int() {
        run_test(&dedent!(
            r#"
                container: "google.protobuf"
                expr: "UInt32Value{value: 34u} == dyn(Int64Value{value: -1})"
                value { bool_value: false }
            "#
        ));
    }

    // Test: uint32_eq_double
    #[should_panic]
    #[test]
    fn uint32_eq_double() {
        run_test(&dedent!(
            r#"
                container: "google.protobuf"
                expr: "UInt32Value{value: 34u} == dyn(DoubleValue{value: 34.0})"
                value { bool_value: true }
            "#
        ));
    }

    // Test: not_uint32_eq_double
    #[should_panic]
    #[test]
    fn not_uint32_eq_double() {
        run_test(&dedent!(
            r#"
                container: "google.protobuf"
                expr: "UInt32Value{value: 34u} == dyn(DoubleValue{value: 18446744073709551616.0})"
                value { bool_value: false }
            "#
        ));
    }

    // Test: uint64
    #[should_panic]
    #[test]
    fn uint64() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_uint64: 9999u}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] { single_uint64: 9999 }
                  }
                }
            "#
        ));
    }

    // Test: sint32
    #[should_panic]
    #[test]
    fn sint32() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_sint32: -3}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] { single_sint32: -3 }
                  }
                }
            "#
        ));
    }

    // Test: sint64
    #[should_panic]
    #[test]
    fn sint64() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_sint64: 255}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] { single_sint64: 255 }
                  }
                }
            "#
        ));
    }

    // Test: fixed32
    #[should_panic]
    #[test]
    fn fixed32() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_fixed32: 43u}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] { single_fixed32: 43 }
                  }
                }
            "#
        ));
    }

    // Test: fixed64
    #[should_panic]
    #[test]
    fn fixed64() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_fixed64: 1880u}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] { single_fixed64: 1880 }
                  }
                }
            "#
        ));
    }

    // Test: sfixed32
    #[should_panic]
    #[test]
    fn sfixed32() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_sfixed32: -404}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] { single_sfixed32: -404 }
                  }
                }
            "#
        ));
    }

    // Test: sfixed64
    #[should_panic]
    #[test]
    fn sfixed64() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_sfixed64: -1}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] { single_sfixed64: -1 }
                  }
                }
            "#
        ));
    }

    // Test: float
    #[should_panic]
    #[test]
    fn float() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_float: 3.1416}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] { single_float: 3.1416 }
                  }
                }
            "#
        ));
    }

    // Test: float_eq_int
    #[should_panic]
    #[test]
    fn float_eq_int() {
        run_test(&dedent!(
            r#"
                container: "google.protobuf"
                expr: "FloatValue{value: 3.0} == dyn(Int64Value{value: 3})"
                value { bool_value: true }
            "#
        ));
    }

    // Test: not_float_eq_int
    #[should_panic]
    #[test]
    fn not_float_eq_int() {
        run_test(&dedent!(
            r#"
                container: "google.protobuf"
                expr: "FloatValue{value: -1.14} == dyn(Int64Value{value: -1})"
                value { bool_value: false }
            "#
        ));
    }

    // Test: float_eq_uint
    #[should_panic]
    #[test]
    fn float_eq_uint() {
        run_test(&dedent!(
            r#"
                container: "google.protobuf"
                expr: "FloatValue{value: 34.0} == dyn(UInt64Value{value: 34u})"
                value { bool_value: true }
            "#
        ));
    }

    // Test: not_float_eq_uint
    #[should_panic]
    #[test]
    fn not_float_eq_uint() {
        run_test(&dedent!(
            r#"
                container: "google.protobuf"
                expr: "FloatValue{value: -1.0} == dyn(UInt64Value{value: 18446744073709551615u})"
                value { bool_value: false }
            "#
        ));
    }

    // Test: double
    #[should_panic]
    #[test]
    fn double() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_double: 6.022e23}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] { single_double: 6.022e23 }
                  }
                }
            "#
        ));
    }

    // Test: bool
    #[should_panic]
    #[test]
    fn bool() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_bool: true}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] { single_bool: true }
                  }
                }
            "#
        ));
    }

    // Test: string
    #[should_panic]
    #[test]
    fn string() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_string: 'foo'}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] { single_string: "foo" }
                  }
                }
            "#
        ));
    }

    // Test: bytes
    #[should_panic]
    #[test]
    fn bytes() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_bytes: b'\\377'}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] { single_bytes: "\377" }
                  }
                }
            "#
        ));
    }
}

// Section: literal_wellknown
// Literals with well-known fields set.
mod literal_wellknown {
    use conformance::runner::run_test;
    use dedent::dedent;

    // Test: any
    #[should_panic]
    #[test]
    fn any() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_any: TestAllTypes{single_int32: 1}}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] {
                      single_any {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] {
                          single_int32: 1
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: duration
    #[should_panic]
    #[test]
    fn duration() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_duration: duration('123s')}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] {
                      single_duration {
                        seconds: 123
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: timestamp
    #[should_panic]
    #[test]
    fn timestamp() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_timestamp: timestamp('2009-02-13T23:31:30Z')}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] {
                      single_timestamp {
                        seconds: 1234567890
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: struct
    #[should_panic]
    #[test]
    fn r#struct() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_struct: {'one': 1, 'two': 2}}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] {
                      single_struct {
                        fields {
                          key: "one"
                          value { number_value: 1.0 }
                        }
                        fields {
                          key: "two"
                          value { number_value: 2.0 }
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: value
    #[should_panic]
    #[test]
    fn value() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_value: 'foo'}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] {
                      single_value {
                        string_value: "foo"
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: int64_wrapper
    #[should_panic]
    #[test]
    fn int64_wrapper() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_int64_wrapper: -321}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] {
                      single_int64_wrapper {
                        value: -321
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: int32_wrapper
    #[should_panic]
    #[test]
    fn int32_wrapper() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_int32_wrapper: -456}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] {
                      single_int32_wrapper {
                        value: -456
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: double_wrapper
    #[should_panic]
    #[test]
    fn double_wrapper() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_double_wrapper: 2.71828}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] {
                      single_double_wrapper {
                        value: 2.71828
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: float_wrapper
    #[should_panic]
    #[test]
    fn float_wrapper() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_float_wrapper: 2.99792e8}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] {
                      single_float_wrapper {
                        value: 2.99792e8
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: uint64_wrapper
    #[should_panic]
    #[test]
    fn uint64_wrapper() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_uint64_wrapper: 8675309u}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] {
                      single_uint64_wrapper {
                        value: 8675309
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: uint32_wrapper
    #[should_panic]
    #[test]
    fn uint32_wrapper() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_uint32_wrapper: 987u}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] {
                      single_uint32_wrapper {
                        value: 987
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: string_wrapper
    #[should_panic]
    #[test]
    fn string_wrapper() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_string_wrapper: 'hubba'}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] {
                      single_string_wrapper {
                        value: "hubba"
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: bool_wrapper
    #[should_panic]
    #[test]
    fn bool_wrapper() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_bool_wrapper: true}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] {
                      single_bool_wrapper {
                        value: true
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: bytes_wrapper
    #[should_panic]
    #[test]
    fn bytes_wrapper() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_bytes_wrapper: b'\\301\\103'}"
                value {
                  object_value {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] {
                      single_bytes_wrapper {
                        value: "\301\103"
                      }
                    }
                  }
                }
            "#
        ));
    }
}

// Section: singular_bind
// Binding the singular fields.
mod singular_bind {
    use conformance::runner::run_test;
    use dedent::dedent;

    // Test: int32
    #[should_panic]
    #[test]
    fn int32() {
        run_test(&dedent!(
            r#"
                expr: "x.single_int32"
                type_env: {
                  name: "x"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "x"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] { single_int32: 17 }
                      }
                    }
                  }
                }
                value: { int64_value: 17 }
            "#
        ));
    }

    // Test: int64
    #[should_panic]
    #[test]
    fn int64() {
        run_test(&dedent!(
            r#"
                expr: "x.single_int64"
                type_env: {
                  name: "x"
                  ident { type { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "x"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes] { single_int64: -99 }
                      }
                    }
                  }
                }
                value: { int64_value: -99 }
            "#
        ));
    }
}

// Section: empty_field
// Tests on empty fields.
mod empty_field {
    use conformance::runner::run_test;
    use dedent::dedent;

    // Test: scalar_with_default
    #[should_panic]
    #[test]
    fn scalar_with_default() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{}.single_int32"
                value: { int64_value: -32 }
            "#
        ));
    }

    // Test: scalar_no_default
    #[should_panic]
    #[test]
    fn scalar_no_default() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{}.single_fixed32"
                value: { uint64_value: 0 }
            "#
        ));
    }

    // Test: nested_message
    #[should_panic]
    #[test]
    fn nested_message() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{}.single_nested_message"
                value: {
                  object_value: {
                    [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes.NestedMessage] {}
                  }
                }
            "#
        ));
    }

    // Test: nested_message_subfield
    #[should_panic]
    #[test]
    fn nested_message_subfield() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{}.single_nested_message.bb"
                value: { int64_value: 0 }
            "#
        ));
    }

    // Test: wkt
    #[should_panic]
    #[test]
    fn wkt() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{}.single_int64_wrapper"
                value: { null_value: NULL_VALUE }
            "#
        ));
    }

    // Test: repeated_scalar
    #[should_panic]
    #[test]
    fn repeated_scalar() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{}.repeated_int64"
                value: { list_value: {} }
            "#
        ));
    }

    // Test: repeated_enum
    #[should_panic]
    #[test]
    fn repeated_enum() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto3"
                expr: "TestAllTypes{}.repeated_nested_enum"
                value: { list_value: {} }
            "#
        ));
    }

    // Test: repeated_nested
    #[should_panic]
    #[test]
    fn repeated_nested() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{}.repeated_nested_message"
                value: { list_value: {} }
            "#
        ));
    }

    // Test: map
    #[should_panic]
    #[test]
    fn map() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{}.map_string_string"
                value: { map_value: {} }
            "#
        ));
    }
}

// Section: has
// Tests for the has() macro on proto2 messages.
mod has {
    use conformance::runner::run_test;
    use dedent::dedent;

    // Test: undefined
    #[test]
    fn undefined() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{}.no_such_field)"
                disable_check: true
                eval_error: {
                  errors: { message: "no_such_field" }
                }
            "#
        ));
    }

    // Test: repeated_none_implicit
    #[should_panic]
    #[test]
    fn repeated_none_implicit() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{}.repeated_int32)"
                value { bool_value: false }
            "#
        ));
    }

    // Test: repeated_none_explicit
    #[should_panic]
    #[test]
    fn repeated_none_explicit() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{repeated_int32: []}.repeated_int32)"
                value { bool_value: false }
            "#
        ));
    }

    // Test: repeated_one
    #[should_panic]
    #[test]
    fn repeated_one() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{repeated_int32: [1]}.repeated_int32)"
                value { bool_value: true }
            "#
        ));
    }

    // Test: repeated_many
    #[should_panic]
    #[test]
    fn repeated_many() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{repeated_int32: [1, 2, 3]}.repeated_int32)"
                value { bool_value: true }
            "#
        ));
    }

    // Test: map_none_implicit
    #[should_panic]
    #[test]
    fn map_none_implicit() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{}.map_string_string)"
                value { bool_value: false }
            "#
        ));
    }

    // Test: map_none_explicit
    #[should_panic]
    #[test]
    fn map_none_explicit() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{map_string_string: {}}.map_string_string)"
                value { bool_value: false }
            "#
        ));
    }

    // Test: map_one_default
    #[should_panic]
    #[test]
    fn map_one_default() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{map_string_string: {'MT': ''}}.map_string_string)"
                value { bool_value: true }
            "#
        ));
    }

    // Test: map_one
    #[should_panic]
    #[test]
    fn map_one() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{map_string_string: {'one': 'uno'}}.map_string_string)"
                value { bool_value: true }
            "#
        ));
    }

    // Test: map_many
    #[should_panic]
    #[test]
    fn map_many() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{map_string_string: {'one': 'uno', 'two': 'dos'}}.map_string_string)"
                value { bool_value: true }
            "#
        ));
    }

    // Test: required
    #[should_panic]
    #[test]
    fn required() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestRequired{required_int32: 4}.required_int32)"
                value { bool_value: true }
            "#
        ));
    }

    // Test: optional_unset_no_default
    #[should_panic]
    #[test]
    fn optional_unset_no_default() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{}.single_sint32)"
                value { bool_value: false }
            "#
        ));
    }

    // Test: optional_set_no_default
    #[should_panic]
    #[test]
    fn optional_set_no_default() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{single_sint32: -4}.single_sint32)"
                value { bool_value: true }
            "#
        ));
    }

    // Test: optional_unset_with_default
    #[should_panic]
    #[test]
    fn optional_unset_with_default() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{}.single_int32)"
                value { bool_value: false }
            "#
        ));
    }

    // Test: optional_set_with_default
    #[should_panic]
    #[test]
    fn optional_set_with_default() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{single_int32: 16}.single_int32)"
                value { bool_value: true }
            "#
        ));
    }

    // Test: optional_set_to_default
    #[should_panic]
    #[test]
    fn optional_set_to_default() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{single_int32: -32}.single_int32)"
                value { bool_value: true }
            "#
        ));
    }

    // Test: optional_message_unset
    #[should_panic]
    #[test]
    fn optional_message_unset() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{}.standalone_message)"
                value { bool_value: false }
            "#
        ));
    }

    // Test: optional_message_set
    #[should_panic]
    #[test]
    fn optional_message_set() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{standalone_message: TestAllTypes.NestedMessage{}}.standalone_message)"
                value { bool_value: true }
            "#
        ));
    }

    // Test: optional_enum_unset
    #[should_panic]
    #[test]
    fn optional_enum_unset() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{}.standalone_enum)"
                value { bool_value: false }
            "#
        ));
    }

    // Test: optional_enum_set
    #[should_panic]
    #[test]
    fn optional_enum_set() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{standalone_enum: TestAllTypes.NestedEnum.BAR}.standalone_enum)"
                value { bool_value: true }
            "#
        ));
    }

    // Test: optional_enum_set_zero
    #[should_panic]
    #[test]
    fn optional_enum_set_zero() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{standalone_enum: TestAllTypes.NestedEnum.FOO}.standalone_enum)"
                value { bool_value: true }
            "#
        ));
    }

    // Test: oneof_unset
    #[should_panic]
    #[test]
    fn oneof_unset() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{}.single_nested_message)"
                value { bool_value: false }
            "#
        ));
    }

    // Test: oneof_other_set
    #[should_panic]
    #[test]
    fn oneof_other_set() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{single_nested_enum: TestAllTypes.NestedEnum.BAZ}.single_nested_message)"
                value { bool_value: false }
            "#
        ));
    }

    // Test: oneof_set
    #[should_panic]
    #[test]
    fn oneof_set() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{single_nested_message: TestAllTypes.NestedMessage{}}.single_nested_message)"
                value { bool_value: true }
            "#
        ));
    }

    // Test: oneof_set_default
    #[should_panic]
    #[test]
    fn oneof_set_default() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "has(TestAllTypes{single_nested_enum: TestAllTypes.NestedEnum.FOO}.single_nested_enum)"
                value { bool_value: true }
            "#
        ));
    }
}

// Section: set_null
mod set_null {
    use conformance::runner::run_test;
    use dedent::dedent;

    // Test: single_message
    #[should_panic]
    #[test]
    fn single_message() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_nested_message: null} == TestAllTypes{}"
                value: { bool_value: true }
            "#
        ));
    }

    // Test: single_any
    #[should_panic]
    #[test]
    fn single_any() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_any: null}.single_any"
                value { null_value: NULL_VALUE }
            "#
        ));
    }

    // Test: single_value
    #[should_panic]
    #[test]
    fn single_value() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_value: null}.single_value"
                value { null_value: NULL_VALUE }
            "#
        ));
    }

    // Test: single_duration
    #[should_panic]
    #[test]
    fn single_duration() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_duration: null} == TestAllTypes{}"
                value: { bool_value: true }
            "#
        ));
    }

    // Test: single_timestamp
    #[should_panic]
    #[test]
    fn single_timestamp() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_timestamp: null} == TestAllTypes{}"
                value: { bool_value: true }
            "#
        ));
    }

    // Test: single_scalar
    #[test]
    fn single_scalar() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_bool: null} == TestAllTypes{}"
                disable_check: true
                eval_error: {
                  errors: { message: "unsupported field type" }
                }
            "#
        ));
    }

    // Test: repeated
    #[test]
    fn repeated() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{repeated_int32: null} == TestAllTypes{}"
                disable_check: true
                eval_error: {
                  errors: { message: "unsupported field type" }
                }
            "#
        ));
    }

    // Test: map
    #[test]
    fn map() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{map_string_string: null} == TestAllTypes{}"
                disable_check: true
                eval_error: {
                  errors: { message: "unsupported field type" }
                }
            "#
        ));
    }

    // Test: list_value
    #[test]
    fn list_value() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{list_value: null} == TestAllTypes{}"
                disable_check: true
                eval_error: {
                  errors: { message: "unsupported field type" }
                }
            "#
        ));
    }

    // Test: single_struct
    #[test]
    fn single_struct() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{single_struct: null} == TestAllTypes{}"
                disable_check: true
                eval_error: {
                  errors: { message: "unsupported field type" }
                }
            "#
        ));
    }
}

// Section: quoted_fields
mod quoted_fields {
    use conformance::runner::run_test;
    use dedent::dedent;

    // Test: set_field_with_quoted_name
    #[should_panic]
    #[test]
    fn set_field_with_quoted_name() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{`in`: true} == TestAllTypes{}"
                value: { bool_value: false }
            "#
        ));
    }

    // Test: get_field_with_quoted_name
    #[should_panic]
    #[test]
    fn get_field_with_quoted_name() {
        run_test(&dedent!(
            r#"
                container: "cel.expr.conformance.proto2"
                expr: "TestAllTypes{`in`: true}.`in`"
                value: { bool_value: true }
            "#
        ));
    }
}

// Section: extensions_has
// Tests for presence checks on proto2 extension fields.
mod extensions_has {
    use conformance::runner::run_test;
    use dedent::dedent;

    // Test: package_scoped_int32
    #[should_panic]
    #[test]
    fn package_scoped_int32() {
        run_test(&dedent!(
            r#"
                expr: "has(msg.`cel.expr.conformance.proto2.int32_ext`)"
                type_env: {
                  name: "msg"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "msg"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes]: {
                          [cel.expr.conformance.proto2.int32_ext]: 42
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: package_scoped_nested_ext
    #[should_panic]
    #[test]
    fn package_scoped_nested_ext() {
        run_test(&dedent!(
            r#"
                expr: "has(msg.`cel.expr.conformance.proto2.nested_ext`)"
                type_env: {
                  name: "msg"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "msg"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes]: {
                          [cel.expr.conformance.proto2.nested_ext]: {}
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: package_scoped_test_all_types_ext
    #[should_panic]
    #[test]
    fn package_scoped_test_all_types_ext() {
        run_test(&dedent!(
            r#"
                expr: "has(msg.`cel.expr.conformance.proto2.test_all_types_ext`)"
                type_env: {
                  name: "msg"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "msg"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes]: {
                          [cel.expr.conformance.proto2.test_all_types_ext]: {}
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: package_scoped_test_all_types_nested_enum_ext
    #[should_panic]
    #[test]
    fn package_scoped_test_all_types_nested_enum_ext() {
        run_test(&dedent!(
            r#"
                expr: "has(msg.`cel.expr.conformance.proto2.nested_enum_ext`)"
                type_env: {
                  name: "msg"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "msg"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes]: {
                          [cel.expr.conformance.proto2.nested_enum_ext]: BAR
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: package_scoped_repeated_test_all_types
    #[should_panic]
    #[test]
    fn package_scoped_repeated_test_all_types() {
        run_test(&dedent!(
            r#"
                expr: "has(msg.`cel.expr.conformance.proto2.repeated_test_all_types`)"
                type_env: {
                  name: "msg"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "msg"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes]: {
                          [cel.expr.conformance.proto2.repeated_test_all_types]: [ { single_int64: 1 }, { single_bool: true } ]
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: message_scoped_int64
    #[should_panic]
    #[test]
    fn message_scoped_int64() {
        run_test(&dedent!(
            r#"
                expr: "has(msg.`cel.expr.conformance.proto2.Proto2ExtensionScopedMessage.int64_ext`)"
                type_env: {
                  name: "msg"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "msg"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes]: {
                          [cel.expr.conformance.proto2.Proto2ExtensionScopedMessage.int64_ext]: 42
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: message_scoped_nested_ext
    #[should_panic]
    #[test]
    fn message_scoped_nested_ext() {
        run_test(&dedent!(
            r#"
                expr: "has(msg.`cel.expr.conformance.proto2.Proto2ExtensionScopedMessage.message_scoped_nested_ext`)"
                type_env: {
                  name: "msg"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "msg"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes]: {
                          [cel.expr.conformance.proto2.Proto2ExtensionScopedMessage.message_scoped_nested_ext]: {}
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: message_scoped_nested_enum_ext
    #[should_panic]
    #[test]
    fn message_scoped_nested_enum_ext() {
        run_test(&dedent!(
            r#"
                expr: "has(msg.`cel.expr.conformance.proto2.Proto2ExtensionScopedMessage.nested_enum_ext`)"
                type_env: {
                  name: "msg"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "msg"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes]: {
                          [cel.expr.conformance.proto2.Proto2ExtensionScopedMessage.nested_enum_ext]: BAR
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: message_scoped_repeated_test_all_types
    #[should_panic]
    #[test]
    fn message_scoped_repeated_test_all_types() {
        run_test(&dedent!(
            r#"
                expr: "has(msg.`cel.expr.conformance.proto2.Proto2ExtensionScopedMessage.message_scoped_repeated_test_all_types`)"
                type_env: {
                  name: "msg"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "msg"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes]: {
                          [cel.expr.conformance.proto2.Proto2ExtensionScopedMessage.message_scoped_repeated_test_all_types]: [ { single_int64: 1 }, { single_bool: true } ]
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }
}

// Section: extensions_get
// Tests for accessing proto2 extension fields.
mod extensions_get {
    use conformance::runner::run_test;
    use dedent::dedent;

    // Test: package_scoped_int32
    #[should_panic]
    #[test]
    fn package_scoped_int32() {
        run_test(&dedent!(
            r#"
                expr: "msg.`cel.expr.conformance.proto2.int32_ext` == 42"
                type_env: {
                  name: "msg"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "msg"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes]: {
                          [cel.expr.conformance.proto2.int32_ext]: 42
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: package_scoped_nested_ext
    #[should_panic]
    #[test]
    fn package_scoped_nested_ext() {
        run_test(&dedent!(
            r#"
                expr: "msg.`cel.expr.conformance.proto2.nested_ext` == cel.expr.conformance.proto2.TestAllTypes{}"
                type_env: {
                  name: "msg"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "msg"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes]: {
                          [cel.expr.conformance.proto2.nested_ext]: {}
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: package_scoped_test_all_types_ext
    #[should_panic]
    #[test]
    fn package_scoped_test_all_types_ext() {
        run_test(&dedent!(
            r#"
                expr: "msg.`cel.expr.conformance.proto2.test_all_types_ext` == cel.expr.conformance.proto2.TestAllTypes{}"
                type_env: {
                  name: "msg"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "msg"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes]: {
                          [cel.expr.conformance.proto2.test_all_types_ext]: {}
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: package_scoped_test_all_types_nested_enum_ext
    #[should_panic]
    #[test]
    fn package_scoped_test_all_types_nested_enum_ext() {
        run_test(&dedent!(
            r#"
                expr: "msg.`cel.expr.conformance.proto2.nested_enum_ext` == cel.expr.conformance.proto2.TestAllTypes.NestedEnum.BAR"
                type_env: {
                  name: "msg"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "msg"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes]: {
                          [cel.expr.conformance.proto2.nested_enum_ext]: BAR
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: package_scoped_repeated_test_all_types
    #[should_panic]
    #[test]
    fn package_scoped_repeated_test_all_types() {
        run_test(&dedent!(
            r#"
                expr: "msg.`cel.expr.conformance.proto2.repeated_test_all_types` == [cel.expr.conformance.proto2.TestAllTypes{single_int64: 1}, cel.expr.conformance.proto2.TestAllTypes{single_bool: true}]"
                type_env: {
                  name: "msg"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "msg"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes]: {
                          [cel.expr.conformance.proto2.repeated_test_all_types]: [ { single_int64: 1 }, { single_bool: true } ]
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: message_scoped_int64
    #[should_panic]
    #[test]
    fn message_scoped_int64() {
        run_test(&dedent!(
            r#"
                expr: "msg.`cel.expr.conformance.proto2.Proto2ExtensionScopedMessage.int64_ext` == 42"
                type_env: {
                  name: "msg"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "msg"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes]: {
                          [cel.expr.conformance.proto2.Proto2ExtensionScopedMessage.int64_ext]: 42
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: message_scoped_nested_ext
    #[should_panic]
    #[test]
    fn message_scoped_nested_ext() {
        run_test(&dedent!(
            r#"
                expr: "msg.`cel.expr.conformance.proto2.Proto2ExtensionScopedMessage.message_scoped_nested_ext` == cel.expr.conformance.proto2.TestAllTypes{}"
                type_env: {
                  name: "msg"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "msg"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes]: {
                          [cel.expr.conformance.proto2.Proto2ExtensionScopedMessage.message_scoped_nested_ext]: {}
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: message_scoped_nested_enum_ext
    #[should_panic]
    #[test]
    fn message_scoped_nested_enum_ext() {
        run_test(&dedent!(
            r#"
                expr: "msg.`cel.expr.conformance.proto2.Proto2ExtensionScopedMessage.nested_enum_ext` == cel.expr.conformance.proto2.TestAllTypes.NestedEnum.BAR"
                type_env: {
                  name: "msg"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "msg"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes]: {
                          [cel.expr.conformance.proto2.Proto2ExtensionScopedMessage.nested_enum_ext]: BAR
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }

    // Test: message_scoped_repeated_test_all_types
    #[should_panic]
    #[test]
    fn message_scoped_repeated_test_all_types() {
        run_test(&dedent!(
            r#"
                expr: "msg.`cel.expr.conformance.proto2.Proto2ExtensionScopedMessage.message_scoped_repeated_test_all_types` == [cel.expr.conformance.proto2.TestAllTypes{single_int64: 1}, cel.expr.conformance.proto2.TestAllTypes{single_bool: true}]"
                type_env: {
                  name: "msg"
                  ident: { type: { message_type: "cel.expr.conformance.proto2.TestAllTypes" } }
                }
                bindings: {
                  key: "msg"
                  value: {
                    value: {
                      object_value: {
                        [type.googleapis.com/cel.expr.conformance.proto2.TestAllTypes]: {
                          [cel.expr.conformance.proto2.Proto2ExtensionScopedMessage.message_scoped_repeated_test_all_types]: [ { single_int64: 1 }, { single_bool: true } ]
                        }
                      }
                    }
                  }
                }
            "#
        ));
    }
}
