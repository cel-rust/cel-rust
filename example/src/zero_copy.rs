...
/// Whether `slice` is a part of `whole`'s bytes, rather than a copy of them.
fn is_slice_of(slice: &str, whole: &str) -> bool {
    let (slice, whole) = (
        slice.as_bytes().as_ptr_range(),
        whole.as_bytes().as_ptr_range(),
    );
    whole.start <= slice.start && slice.end <= whole.end
}
...
fn main() {
    ...
    assert!(is_slice_of(method, request.method));

    // The version-stripped path is a slice of the request's path, not a copy of it.
    let stripped = evaluate("request.stripVersion()");
    println!("stripped path: {stripped}");
    assert_eq!(stripped, "/users/42");
    assert!(is_slice_of(stripped, &request.path));
    ...
    assert!(is_slice_of(host, &request.headers[0].1));
}
